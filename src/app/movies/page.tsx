...
export default function MoviesPage() {
  ...
  // Фильтруем данные только для фильмов
  const movies = useMemo(() => {
    return moviesData.filter((item) => item.type === "фильм");
  }, []);
  ...
  const years = useMemo(() => {
    const set = new Set<string>();
    movies.forEach((item) => set.add(item.year));
  ...
  return (
    <main className="min-h-screen pt-28 pb-20 bg-[#0F0F11]">
      <div className="container">
        {/* Заголовок страницы */}
        <div className="mb-8">
          <h1 className="text-4xl font-black mb-2">Фильмы</h1>
          <p className="text-gray-400">Найдено фильмов: {filteredMovies.length}</p>
        </div>
        ...
        {filteredMovies.length > 0 ? (
          ...
          <div className="text-center py-20 text-gray-500 bg-[#131316] rounded-2xl border border-white/5 p-8">
            ...
            <p className="text-xl font-bold text-white mb-2">Фильмы не найдены</p>
